-- docs/DATABASE_STRUCTURE_FIX.sql
-- Vuet App - Comprehensive Database Setup Script (FIXED version for 'owner' column)
-- Version 1.2 - 2025-06-27
-- This script addresses the "column e.owner does not exist" error by correcting RLS policies
-- to use 'user_id' instead of 'owner' in the 'entities' table reference.
-- It also includes all previous fixes for missing tables and incompatible types.
...
-- Create 'entity_members' table (FIXED: This table was missing in previous versions)
...
--------------------------------------------------------------------------------
-- SECTION 6: Define RLS Policies (FIXED: Using 'user_id' instead of 'owner')
--------------------------------------------------------------------------------
...
for select using (
  exists (
    ...
    -- FIXED: Using e.user_id instead of e.owner
...
-- Policies for 'entity_members' table:
...
for select using (member_id = auth.uid() or exists (select 1 from public.entities where id = entity_id and user_id = auth.uid()));
...
for insert with check (exists (select 1 from public.entities where id = entity_id and user_id = auth.uid()));
...
for delete using (exists (select 1 from public.entities where id = entity_id and user_id = auth.uid()));
...
--------------------------------------------------------------------------------
-- Uncomment to check entities table structure
-- select column_name, data_type 
-- from information_schema.columns 
-- where table_schema = 'public' and table_name = 'entities';

-- Uncomment to check if user_id column exists in entities table
-- select exists(
--   select 1 
--   from information_schema.columns 
...
-- Uncomment to check tag count
-- select count(*) from public.tags;

-- Uncomment to check entity_members table
-- select exists(
--   select 1 
--   from information_schema.tables 
--   where table_schema = 'public' 
--     and table_name = 'entity_members'
-- ) as entity_members_exists;

-- Uncomment to check RLS policies
-- select tablename, policyname 
-- from pg_policies 
-- where schemaname = 'public' 
--   and (tablename = 'tags' or tablename = 'entity_tags' or tablename = 'entity_members');
