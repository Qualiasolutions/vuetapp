-- Simplified authentication system - profiles table only
-- This migration only affects the profiles table and auth triggers

-- 1. Drop existing auth trigger and profile-specific function
...
-- 8. Create robust profile creation trigger function
...
-- Use existing update_updated_at_column function for profiles updated_at trigger
CREATE TRIGGER update_profiles_updated_at
    BEFORE UPDATE ON public.profiles
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 10. Ensure service_role can bypass RLS when needed
