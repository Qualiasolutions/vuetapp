...
/// Implementation of the TaskRepository interface using Supabase
class SupabaseTaskRepository implements TaskRepository {
  ...
  Future<List<TaskModel>> getSubtasks(String parentTaskId) async {
    try {
      ...
      ErrorHandler.handleError('Error getting subtasks', e);
      return [];
    }
  }
  ...
  Future<List<TaskModel>> getTasksSharedWithMe() async {
    try {
      ...
      final sharingResponse = await _supabase
          .from(_taskSharingTable)
          .select('task_id')
          .eq('user_id', userId);
...
final supabaseTaskRepositoryProvider = Provider<TaskRepository>((ref) {
  ...
  return SupabaseTaskRepository(supabase: supabase);
});
