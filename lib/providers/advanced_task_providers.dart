...
final createFlexibleTaskProvider =
    FutureProvider.family<FlexibleTask, FlexibleTask>((ref, task) async {
...
final createFixedTaskProvider =
    FutureProvider.family<FixedTask, FixedTask>((ref, task) async {
  final repository = ref.watch(advancedTaskRepositoryProvider);
  return await repository.createFixedTask(task);
});

// Provider for getting flexible tasks by urgency
final flexibleTasksByUrgencyProvider =
    FutureProvider.family<List<FlexibleTask>, enums.TaskUrgency>(
        (ref, urgency) async {
  final repository = ref.watch(advancedTaskRepositoryProvider);
...
final unscheduledFlexibleTasksProvider =
    FutureProvider<List<FlexibleTask>>((ref) async {
  final repository = ref.watch(advancedTaskRepositoryProvider);
  final authService = ref.watch(authServiceProvider);
  final user = authService.currentUser;
  if (user == null) throw Exception('User not authenticated');
...
final fixedTasksInRangeProvider =
    FutureProvider.family<List<FixedTask>, DateRange>((ref, dateRange) async {
  final repository = ref.watch(advancedTaskRepositoryProvider);
  final authService = ref.watch(authServiceProvider);
  final user = authService.currentUser;
  if (user == null) throw Exception('User not authenticated');

  return await repository.getFixedTasksInRange(
      user.id, dateRange.start, dateRange.end);
});

// Provider for detecting scheduling conflicts
final schedulingConflictsProvider =
    FutureProvider.family<List<FixedTask>, ConflictCheck>(
        (ref, conflictCheck) async {
  final repository = ref.watch(advancedTaskRepositoryProvider);
  final authService = ref.watch(authServiceProvider);
  final user = authService.currentUser;
  if (user == null) throw Exception('User not authenticated');
...
final scheduleFlexibleTaskProvider =
    FutureProvider.family<FlexibleTask, SmartScheduleRequest>(
        (ref, request) async {
  final repository = ref.watch(advancedTaskRepositoryProvider);
...
final allAdvancedTasksProvider =
    FutureProvider<List<AdvancedTask>>((ref) async {
  final repository = ref.watch(advancedTaskRepositoryProvider);
  final authService = ref.watch(authServiceProvider);
  final user = authService.currentUser;
  if (user == null) throw Exception('User not authenticated');
...
class ConflictCheck {
  ...
  @override
  int get hashCode =>
      startDateTime.hashCode ^ endDateTime.hashCode ^ excludeTaskId.hashCode;
}
...
class SmartScheduleRequest {
  ...
  @override
  int get hashCode =>
      taskId.hashCode ^ scheduledStartTime.hashCode ^ scheduledEndTime.hashCode;
}
